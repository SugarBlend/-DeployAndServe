...
class TritonRemote(ABC):
    ...
    async def initialize(self) -> None:
        if self.triton_client is None:
            ...
            self.outputs = {
                node["name"]: client.InferRequestedOutput(node["name"]) for node in self.metadata["outputs"]
            }
    ...
    async def infer(self, feed_input: List[np.ndarray]) -> List[np.ndarray]:
        ...
        for idx, node in enumerate(self.metadata["inputs"]):
            ...
            inputs.append(
                self.client.InferInput(node["name"], list(feed_input[idx].shape), node["datatype"]).set_data_from_numpy(
                    data
                )
            )

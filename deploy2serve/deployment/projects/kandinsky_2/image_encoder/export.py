...
from deploy2serve.deployment.projects.kandinsky_2.image_encoder.batcher import ImageEncoderBatcher
...
class OverrideONNX(ONNXExporter):
    def register_batcher(self) -> Optional[BaseBatcher]:
        return ImageEncoderBatcher(self.config, "kandinsky-image_encoder", 1)
...
class ModelWrapper(torch.nn.Module):
    ...
    def forward(self, *args, **kwargs) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        return out.last_hidden_state, out.image_embeds


class ImageEncoderExporter(Exporter):
    def load_checkpoints(self, config_path: str, weights_path: str) -> torch.nn.Module:
        if self.config.enable_mixed_precision:
            dtype = torch.float16
        else:
            dtype = torch.float32
        image_encoder = KandinskyV22PriorPipeline.from_pretrained(
            "kandinsky-community/kandinsky-2-2-prior",
            torch_dtype=dtype, cache_dir="/tmp/kandinsky2",
            low_cpu_mem_usage=True
        ).image_encoder
        model = ModelWrapper(image_encoder)

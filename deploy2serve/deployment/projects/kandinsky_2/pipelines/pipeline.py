import numpy as np
from diffusers.pipelines.kandinsky2_2.pipeline_kandinsky2_2 import (downscale_height_and_width, ImagePipelineOutput,
                                                                    deprecate, KandinskyV22Pipeline, UNet2DConditionModel,
                                                                    DDPMScheduler, VQModel)
import torch
...
class Pipeline(KandinskyV22Pipeline,  metaclass=LoggingMeta):
    ...
    @classmethod
    def from_pretrained(cls, pretrained_model_name_or_path, **kwargs) -> "Pipeline":
        original_pipe = KandinskyV22Pipeline.from_pretrained(
    ...
    def __call__(
        ...
        **kwargs,
    ) -> Union[ImagePipelineOutput, Tuple[np.ndarray, ...]]:
        """
        ...
        for i, t in enumerate(self.progress_bar(timesteps)):
            ...
            noise_pred = self.unet.infer(
                {"latent": latent_model_input, "timestep": t.reshape(-1), "image_embeds": image_embeds}
            )[0]
        ...
        if not output_type == "latent":
            # post-processing
            image = self.movq.infer({"latent": latents})[0]
